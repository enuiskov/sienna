		...
		protected override void OnMouseMove(MouseEventArgs iEvent)
		{
			...
			if(iEvent.Button == MouseButtons.Left)
			{
				...
				var _ScrollOffs = this.CurrentDocument.Scroll.Offset; 
				{
					...
					(
						_ScrollOffs.X + (int)Math.Round((iEvent.X - ((double)this.Settings.CharWidth  / 2)) / this.Width  * this.BufferSize.Width) - this.CurrentDocument.LineNumberOffset,
						_ScrollOffs.Y + (int)Math.Round((iEvent.Y - ((double)this.Settings.LineHeight / 2)) / this.Height * this.BufferSize.Height)
					);
