...
namespace AE.Data.DescriptionLanguage
{
	...
	public partial class ExecutionContext
	{
		...
		public int GetIdentifierOffset(TokenInfo iToken)
		{
			...
			var _AbsOffset = this.FramePointer + _SigItem.BaseOffset;
		...
		public void InvokeReturn()
		{
			...
			{
				var _OSMCallInfo = (CallInfo)(_DataStack.Peek().Value);
		...
		public int ExecuteInstruction(AEDLOpcode iOpcode)
		{
			...
			else if(iOpcode.Type == AEDLOpcodeType.Breakpoint)
			{
				...
				else
				{
					...
					var _TOS   = _DataStack.Position < (_DataStack.Items.Length)     ? _DataStack[0].Value : null;
					var _NOS   = _DataStack.Position < (_DataStack.Items.Length - 1) ? _DataStack[1].Value : null;
					...
					else if(iOpcode.Type == AEDLOpcodeType.PushLabelPointer)
					{
						...
						_DataStack.Push(_Label.Position);
					}
					else switch(_OpcodeName)
					{
						...
						case "get_IP" : 
						{
							_DataStack.Push(this.Program.Counter);
							break;
						}
						case "set_IP" : 
						{
							var _SpecIP = (int)(_DataStack.Peek().Value);
						...
						case "set_SP" : 
						{
							var _SpecSP = (int)(_DataStack.Peek().Value);
							_DataStack.Position = _SpecSP;
						...
						case "get_FP" : 
						{
							_DataStack.Push(this.FramePointer);

							break;
						}
						case "set_FP" : 
						{
							var _SpecFP = (int)(_DataStack.Peek().Value);
						...
						case "set_ident_name" : 
						{
							...
							var _IdentOffset = (int)(_DataStack[1].Value);
							
							_DataStack[_IdentOffset].Name = _IdentName;
						...
						case "get-node-child":
						{
							...
							var _ChildNodeIndex = (int)_TOS;
							
							_DataStack.Drop();
							_DataStack.Push(_ParentNode.Children[_ChildNodeIndex]);

							break;
						}
						case "get-node-children-count":
						{
						...
						case "create-list":
						{
							var _ItemCount = (int)_TOS;
							var _NewList = new ListData(){Items = new List<ListItemData>(new ListItemData[_ItemCount])};

							_DataStack.Drop();
							_DataStack.Push(_NewList);

							break;
						}
						case "get-list-length" :
						{
							var _List = (ListData)_TOS;
							_DataStack.Push(_List.Items.Count);
							break;
						}
						case "get-list-item" :
						{
							var _List = (ListData)_NOS;
							var _ItemIndex = (int)_TOS;

							_DataStack.Drop();
							_DataStack.Push(_List.Items[_ItemIndex].Value);

							break;
						}
						case "set-list-item" :
						{
							var _List = (ListData)(_DataStack[2].Value);
							var _Index = (int)_NOS;
							var _Value = _TOS;
							
							_List.Items[_Index] = new ListItemData{Value = _Value};
							_DataStack.Drop();
						...
						case "set-return-value":
						{
							var _RetList = (ListData)_TOS;
							this.CurrentScope.ReturnValue = _RetList;
